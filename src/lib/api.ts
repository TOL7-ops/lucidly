...
import { Sentiment } from './supabaseClient'
...
export const dreamsApi = {
  ...
  getDreamById: async (id: string): Promise<ApiResponse<Dream>> => {
    return apiRequest<Dream>(`/dreams/${id}`);
  },
  ...
  createDream: async (dream: {
    ...
    // Log outgoing payload for diagnostics
    try {
      const keys = Object.keys(dream || {});
      console.log('[api] createDream payload keys:', keys);
    } catch {}
    return apiRequest<Dream>('/dreams', {
      method: 'POST',
...
export const transcriptionApi = {
  ...
  transcribeAudio: async (audioBlob: Blob): Promise<ApiResponse<{ transcript: string }>> => {
    try {
      ...
      const supportedFormats = ['audio/wav', 'audio/webm', 'audio/mpeg', 'audio/mp4'];
      if (!supportedFormats.some(format => audioBlob.type.includes(format.split('/')[1]))) {
        console.warn('⚠️ Unsupported audio format:', audioBlob.type);
        return {
          success: false,
          error: `Unsupported audio format: ${audioBlob.type}. Please use WAV, WebM, MP3, or MP4.`,
        };
      }

      // Ensure minimum audio size (at least 1KB)
      ...
      // Use the original filename or default to recording.wav
      const filename = `recording.${audioBlob.type.split('/')[1] || 'wav'}`;
      formData.append('file', audioBlob, filename);

      console.log('📤 Sending FormData with field "file" and filename:', filename);
      ...
      try {
        ...
        return {
          success: false,
          error: `Server returned invalid JSON. Status: ${response.status}. Response: ${textResponse.substring(0, 200)}...`,
        };
      }
      ...
      if (!response.ok) {
        ...
        if (response.status === 413) {
          return { success: false, error: 'Audio file is too large. Please try a shorter recording.' };
        }
        if (response.status === 415) {
          return { success: false, error: 'Unsupported audio format. Please use WAV or MP3.' };
        }
        if (response.status === 408) {
          return { success: false, error: 'Transcription timed out. Please try a shorter recording.' };
        }
      ...
      // Normalize transcript across possible server shapes
      let transcript = '';
      if (typeof (data?.data?.transcript) === 'string') {
        transcript = data.data.transcript.trim();
      ...
      if (!transcript) {
        try {
          console.warn('❌ No transcript in response. Full payload:', JSON.stringify(data).substring(0, 2000));
        } catch {
          console.warn('❌ No transcript in response (non-serializable payload).');
        }
        return {
          success: false,
          error: 'No speech detected in your recording. Please try again.',
        };
      }
      ...
      if (error instanceof TypeError && error.message.includes('fetch')) {
        return {
          success: false,
          error: 'Network error: Unable to connect to transcription service. Please check your internet connection.',
        };
      }
...
export const getMoodFromSentiment = (sentiment?: Sentiment): Dream['mood'] => {
